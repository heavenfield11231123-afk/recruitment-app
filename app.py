...
--- ★ここから下の指示を付け足します！ ★ ---
【出力トーンと文字数の指定】
- 表現は常に「下手目（謙虚・低姿勢）」にしてください。上から目線や指導的な表現（〜すべき、〜しなさい等）は禁止です。「もしよろしければご参考までに」など物腰柔らかく。
- 全体的に文字数を少なめにし、箇条書きを活用して簡潔に出力してください。

【受付ブロック回避トークの追加】
- トークの中に、受付突破用の回避トーク（「資料だけお送りしたいのでご担当者様のお名前を…」など）を別立てで含めてください。
-------------------------------------------------
"""
